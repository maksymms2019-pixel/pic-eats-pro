...
-1. АНТИ-ГАЛЮЦИНАЦІЯ (НАЙВАЖЛИВІШЕ): рахуй ТІЛЬКИ те, що РЕАЛЬНО ВИДНО на фото. ЗАБОРОНЕНО додавати продукти "за асоціацією" (бутерброд → НЕ дописуй печиво/каву/фрукти поряд; салат → НЕ дописуй хліб; кава → НЕ дописуй цукор якщо його не видно). Якщо не впевнений що предмет їстівний або є на фото — НЕ включай його. Краще пропустити, ніж вигадати.
...
1. Перерахуй ВСІ ВИДИМІ компоненти страви окремо (тісто/хліб, м'ясо, овочі, сир, соус, олія тощо). Підкреслюю: ВИДИМІ. Якщо щось закрите іншою їжею — постав assumption замість того щоб вигадувати точну вагу.
...
8. items[] використовуй ТІЛЬКИ якщо на тарілці чітко ВИДНО кілька різних страв/продуктів (наприклад бутерброд + яблуко + кава у склянці поруч). Для одного блюда — НЕ заповнюй items, лиши головні поля. НЕ додавай у items нічого, чого нема на фото.
